...
#光、土壌水分、N量、平均気温を線形結合、個体数を誤差分布ポアソン分布に従って生成
...
image(matrix(N[,1],ncol=100,nrow=100),col=terrain.colors(10),main="TRUE_NUNBER_OF_SPECIES")
...
###真の在のみデータでGLM
pred.glmt<-cbind(N.ex,pred)
colnames(pred.glmt)<-c("true_exist",colnames(pred.glmt[,2:8]))
#GLM
result.glmt <- glm(true_exist ~. , family = binomial, data = pred.glmt)
result.glmt
#stepAIC
library(MASS)
result.stepAICt <- stepAIC(result.glmt)
summary(result.stepAICt)
...
#除歪対応分析。第一主成分とかで高く寄与しているものは削りたくない。同じ主成分で寄与している者同士の中で削っていく
library(vegan)
result.dca <- decorana(pred)
result.dca$cproj
sco<-scores(result.dca,display="sites") #地点の序列化スコア
clus<-hclust(dist(sco),"ward") #ウォード法で
### グループ分けした序列を　グラフ表示 ###
windows()
k<-cutree(clus,gnum_st)
ordipointlabel(result.dca,display="species",col = c(2:(gnum_st+2))[k],pch = c(1:gnum_st+1)[k])
