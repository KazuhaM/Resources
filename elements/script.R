...
d<-read.csv("species2.csv",row.names=1)
d[is.na(d)]<-0
d<-t(d)
#d<-t(d)
col<-ncol(d)
...
library(vegan) 
library(labdsv) #indval用
d<-read.csv("envirnment.csv",row.names=1)
d[is.na(d)]<-0
d<-t(d)
#d<-t(d)
col<-ncol(d)
...
d.prop<-c()
for (i in 1 : 4 ){
	d.prop[i]<-d.var$CA$eig[i]/d.var$tot.chi
}
d.prop

# DCAの場合
d.mds<-decorana(d, iresc = 30)
#MDSの場合
