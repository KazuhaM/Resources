...
if (enn == 5) {
	...
	for (j in 1:ja){
		...
		print(sd)
		if (sd < 0.001 ){
			text(x = px[i,] , y = xh-h+cs, labels =paste("***" ,signif(sd,digits=4))  , font = 1, ps = ft, adj = 0)
		}else if(sd < 0.01 ){
			text(x = px[i,] , y = xh-h+cs, labels =paste("**" ,signif(sd,digits=4)), font = 1, ps = ft, adj = 0)
		}else if(sd < 0.05 ){
			text(x = px[i,] , y = xh-h+cs, labels =paste("*" ,signif(sd,digits=4)) , font = 1, ps = ft, adj = 0)
		}else{
			text(x = px[i,] , y = xh-h+cs, labels =signif(sd,digits=4) , cex = 0.8 , font = ft, ps = 8, adj = 0)
		} 	
	}
	}
}else {
