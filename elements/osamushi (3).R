...
ar<-length(abd)
ar
a1r<-length(abd[abd==1])
a2r<-length(abd[abd==2])
a.richness.estimated<-ar + ((as-1)/as)*((a1r*(a1r-1))/(2*(a2r+1)))
a.richness.estimated

a.PIE<-as/(as-1)*(1-sum((abd/as)^2))
a.PIE

a.samp<-c()
for(i in 1:ar){
	a.samp<-c(a.samp,rep(i,abd[i]))
}

# 希薄化（Rarefaction）
num.resamp <- 100 #　繰り返し数
## Rarefaction for アリ
out <- quantile(1, c(0.025, 0.5, 0.975))
for(i in 2:as){
	resamp <- sapply(1:num.resamp, function(x) length( levels( factor( sample(a.samp, i) ) ) ))
