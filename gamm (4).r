...
#データ読み込み
e.data <- read.csv("coverprandabsnonanu.csv",header = T)
head(e.data)
...
#GAMM
gam.model <- gam(ErosionPin~s(Cover), data=e.data,random =QuadratID )    
summary(gam.model)
#GLM
lm.model <- gam(ErosionPin~Cover, data=e.data,random =QuadratID)
summary(lm.model)

#GAMMをプロット
plot(gam.model, residuals=T, pch="。", se=T, main="Spline smoothing non Annual herbs",xlab="Cover(%)",ylab="Erosion Pin(cm)")
