...
flname<-paste(A[a],"-",B[b],"5.csv", sep ="")
...
if (enn == 5) {
	...
	for (j in 1:ja){
		h<-1.2 *ha[i,j]
		segments(x0 = px[i,]-0.2, y0 = xh -1 - h, x1 = px[i,]-0.2, y1 = xh- h, lwd=0.5) 
		segments(x0 = px[i,]-0.2 , y0 = xh- h, x1 = px[i+j,]+0.2, y1 = xh- h, lwd=0.5)
		segments(x0 = px[i+j,]+0.2 , y0 = xh- h, x1 = px[i+j,]+0.2, y1 =xh -1 - h, lwd=0.5)
		print(ha[i,j])
		hb<- ha[i,j]+1
		sd <- SD[hb]
		print(sd)
