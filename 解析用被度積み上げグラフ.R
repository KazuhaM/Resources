...
barplot(
  z,                                                 # 行列型データ
  ylim = c(0,mx),
  #col=gray(0.2*nrow(z)),                          # 色の指定
  legend.text = labels,                                 # 棒グラフに凡例を付ける
  args.legend = list(x = 5, y = mx),     # 凡例の位置を調整
  ylab = "Each Cover", xlab = "Year"
)
		dev.copy(pdf, file="Cs-Y5.pdf", width = 10, height = 10)
		dev.off()
...
px <- barplot(x.mean, ylab = yl, xlab = xl, ylim = c(0,xh+mxs))
...
if (enn == 5) {
	...
	for (j in 1:ja){
		...
		print(sd)
		if (sd < 0.001 ){
			text(x = px[i,] , y = xh-h+cs, labels =paste("***" ,signif(sd,digits=4)) , cex = 0.8 , font = 1, ps = 8, adj = 0)
		}else if(sd < 0.01 ){
			text(x = px[i,] , y = xh-h+cs, labels =paste("**" ,signif(sd,digits=4)), cex = 0.8 , font = 1, ps = 8, adj = 0)
		}else if(sd < 0.05 ){
			text(x = px[i,] , y = xh-h+cs, labels =paste("*" ,signif(sd,digits=4)), cex = 0.8 , font = 1, ps = 8, adj = 0)
		}else{
			text(x = px[i,] , y = xh-h+cs, labels =signif(sd,digits=4) , cex = 0.8 , font = 1, ps = 8, adj = 0)
		} 	
	}
	}
}else {
	...
	for (j in 1:ja){
		...
		dev.copy(pdf, file=paste(flname,"5.pdf",sep=""), width = 10, height = 10)
		dev.off()
